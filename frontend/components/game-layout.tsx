...
export default function GameLayout({
  ...
  useEffect(() => {
    const syncDisplayName = () => {
      const storedName = window.localStorage.getItem("opacity-player-name");
      setDisplayName(storedName && storedName.trim() ? storedName.trim() : username);
    };

    syncDisplayName();
    window.addEventListener("player-name-changed", syncDisplayName);
    window.addEventListener("storage", syncDisplayName);

    return () => {
      window.removeEventListener("player-name-changed", syncDisplayName);
      ...
      <div className="flex flex-col items-center w-full max-w-5xl min-h-screen border-x-0 sm:border-x-2 border-[#efefef]">
        <div className="flex flex-row items-center justify-between w-full px-4 sm:px-5 py-3 border-b-2 border-[#efefef]">
          ...
          <div className="relative shrink-0" ref={userMenuRef}>
            ...
            >
              <Blobatar name={displayName} size={30} animate="always" expression={surprised} className="shrink-0" />
              <span className="font-pixel text-[16px] sm:text-[20px] ml-[5px]">{displayName}</span>
            </button>
            ...
            {userMenuOpen && (
              <div className="absolute top-[calc(100%+6px)] right-0 z-50 w-52 bg-white border border-[#e0e0e0] rounded-[14px] shadow-xl p-1.5 flex flex-col gap-0.5 animate-in fade-in-0 zoom-in-95">
                <div className="px-2.5 py-2 border-b border-[#efefef] mb-0.5">
                  ...
                  <p className="font-pixel text-sm text-[#1d1d1d] truncate mt-0.5">{displayName}</p>
                </div>
