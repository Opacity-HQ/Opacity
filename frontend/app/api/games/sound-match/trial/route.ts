...
import { gradeTrial, type SMPlan, type SMResponse } from "../plan";
...
// Batch trial submission. Grades every submitted trial against the
// server-authored plan in session.config — the client's response carries
// only what was selected, never a claimed correctness.
  ...
  try {
    ...
    for (const submission of submitted) {
      ...
      const { isCorrect, errorType } = gradeTrial(
        trial,
        submission.response as SMResponse,
      );
      ...
      rows.push({
        ...
        // Warm-up trials are recorded but excluded from scoring — same as
        // Rapid Match / Letter Detective.
