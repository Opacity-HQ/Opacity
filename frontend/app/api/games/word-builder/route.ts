...
// Starts a Word Builder session: reads the child's current mastery for this
// skill, picks a difficulty tier accordingly, generates the server-authored
...
export async function POST(request: NextRequest) {
  try {
    ...
    const plan = generatePlan(difficultyLevel);
    ...
    return apiSuccess({ sessionId: session.id, trials: plan.trials });
  } catch (err) {
