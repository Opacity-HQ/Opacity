...
import { gradeTrial, type WBPlan, type WBResponse } from "../plan";

const responseSchema = z.object({
  kind: z.literal("build"),
  tileIndices: z.array(z.number().int().min(0)),
});
...
// Client buffers responses and flushes every ~5 trials + on completion (see
// docs/saket/TRD.md "Timing methodology") rather than one request per trial.
// Every trial is graded here, against the plan stored at session start — the
// client can only ever assert which tiles it tapped and when, never whether
// the resulting word was correct.
export async function POST(request: NextRequest) {
  try {
    ...
    const plan = session.config as unknown as WBPlan;
    ...
    for (const submission of submitted) {
      ...
      const { isCorrect, errorType } = gradeTrial(
        trial,
        submission.response as WBResponse,
      );
