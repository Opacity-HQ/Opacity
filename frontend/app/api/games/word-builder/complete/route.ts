...
import { classifyOutcome, type WBTrial, type WBResponse } from "../plan";

const SKILL_KEY = "word_decoding";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: session, error: sessionError } = await supabase
      .from("game_sessions")
      .select("id, child_id, status, difficulty_level, started_at")
      .eq("id", sessionId)
      .maybeSingle();
    ...
    const { data: trials, error: trialsError } = await supabase
      .from("game_trials")
      .select("is_correct, stimulus, response, reaction_time_ms")
    ...
    // Recomputed from the stored stimulus + response rather than read off
    // the `error_type` column — that column is constrained to Letter
    // Detective's vocabulary (see plan.ts's DB_SAFE_ERROR_TYPE comment) and
    // can't hold "transposition", so the full breakdown lives here instead.
    const detailedOutcomes = scored.map((t) =>
      classifyOutcome(t.stimulus as unknown as WBTrial, t.response as unknown as WBResponse),
    );
    const reversalErrors = detailedOutcomes.filter(
      (o) => o.detailedErrorType === "reversal",
    ).length;
    const transpositionErrors = detailedOutcomes.filter(
      (o) => o.detailedErrorType === "transposition",
    ).length;
    ...
    const reversalErrorRate = scored.length > 0 ? reversalErrors / scored.length : 0;
    const transpositionErrorRate =
      scored.length > 0 ? transpositionErrors / scored.length : 0;
    const throughput = durationMs > 0 ? correctCount / (durationMs / 60000) : 0;
    ...
    const { error: scoreError } = await supabase.from("session_scores").upsert({
      ...
      rt_cv: rtCv,
      // Reused for Word Builder's letter-reversal (b/d, p/q) rate — the same
      // directional/visual confusion concept this column was named for in
      // Letter Detective.
      mirror_error_rate: reversalErrorRate,
      throughput,
      raw_features: {
        version: 1,
        difficultyTier: session.difficulty_level,
        trialsScored: scored.length,
        transpositionErrorRate,
        errorTypeCounts: countBy(
          detailedOutcomes.map((o) => (o.isCorrect ? "correct" : o.detailedErrorType)),
        ),
      },
    });
