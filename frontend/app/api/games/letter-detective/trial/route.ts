...
import { gradeTrial, type LDPlan, type LDResponse } from "../plan";

const responseSchema = z.discriminatedUnion("kind", [
  z.object({ kind: z.literal("lineup"), selectedOptionIndex: z.number().nullable() }),
  z.object({ kind: z.literal("impostor"), selectedIndex: z.number().nullable() }),
  z.object({ kind: z.literal("stakeout"), tapped: z.boolean() }),
  z.object({ kind: z.literal("words"), tappedPositions: z.array(z.number()) }),
]);
...
// Client buffers responses and flushes every ~5 trials + on completion
// (see docs/saket/TRD.md "Timing methodology") rather than one request per
// trial, so network latency doesn't leak into the reaction-time data. Every
// trial is graded here, against the plan stored at session start — the
// client can only ever assert what was selected and when, never whether it
// was correct.
  ...
  try {
    ...
    for (const submission of submitted) {
      ...
      const { isCorrect, errorType } = gradeTrial(
        trial,
        submission.response as LDResponse,
        plan.pair.confusionType,
      );
